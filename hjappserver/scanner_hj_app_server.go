...
type THjAppServerError struct {
	ID       int    `gorm:"primaryKey"`
	Message  string `gorm:"type:text"`
	FailedAt string
}

type THjAppServerScannedLog struct {
	ID       int    `gorm:"primaryKey"`
	FileName string `gorm:"type:text"`
}

func IsScanned(fileName string, db *gorm.DB) (scanned bool, err error) {
	sql := `CREATE TABLE t_hj_app_server_scanned_logs (
		id INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
		file_name TEXT NOT NULL, 
		scanned_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
	);`
	if !db.Migrator().HasTable("t_hj_app_server_scanned_logs") {
		db.Exec(sql)
	}
	var existing THjAppServerScannedLog
	if err := db.Model(THjAppServerScannedLog{}).Where("file_name = ?", fileName).First(&existing).Error; err != nil {
	...
	scanned_log := THjAppServerScannedLog{
		FileName: fileName,
	}
...
func GzippedLogFileReader(logURL string, db *gorm.DB) error {
	db.AutoMigrate(&THjAppServerError{})
