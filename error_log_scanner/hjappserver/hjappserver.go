...
func IsScanned(fileName string, db *gorm.DB) (scanned bool, err error) {
	...
	var existing model.HjAppServerScannedLogs
	if err := db.Model(model.HjAppServerScannedLogs{}).Where("file_name = ?", fileName).First(&existing).Error; err != nil {
	...
	scanned_log := model.HjAppServerScannedLogs{
		FileName: fileName,
	}
...
func GzippedLogFileReader(logURL string, db *gorm.DB) error {
	db.AutoMigrate(&model.HjAppServerErrors{})
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "error") && !strings.Contains(line, "goroutine") && !stackFlag {
			...
			if len(match) > 0 {
				...
				logError := model.HjAppServerErrors{
					Message:  message,
					FailedAt: timestamp,
					Hash:     hash,
				}
